...
const editNoteService = async(notesDispatch, note) => {
    ...
    try {
        const res = await axios.post(`/api/notes/${note._id}`, noteToSend, {headers: Headers});
        notesDispatch({ type: "EDIT_NOTE", payload: {note: res.data.notes}})
    } catch(e) {
